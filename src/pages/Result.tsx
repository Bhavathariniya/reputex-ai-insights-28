    ...
    const fetchTokenData = async () => {
      try {
        ...
        if (tokenInfoData.status === '1' && tokenInfoData.result.length > 0) {
          ...
          tokenInfo = {
            tokenName: result.tokenName || sourceCodeData.result[0]?.ContractName || 'Unverified Contract',
          ...
          tokenInfo = {
            ...tokenInfo,
            tokenName: result.ContractName || 'Unverified Contract',
        ...
        const analysisScores = {
          trust_score: Math.min(95, Math.floor(Math.random() * 30) + 70),
          developer_score: Math.min(90, Math.floor(Math.random() * 20) + 80),
          ...
          fraud_risk: Math.min(15, Math.floor(Math.random() * 20)),
          social_sentiment: Math.floor(Math.random() * 20) + 70,
          confidence_score: sourceCodeData.status === '1' ? 90 : 60
        };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="text-sm text-muted-foreground px-3 py-1.5 bg-muted rounded-full">
              Network: {network}
            </div>
          </div>

          <div className="bg-card rounded-lg p-4 break-all">
